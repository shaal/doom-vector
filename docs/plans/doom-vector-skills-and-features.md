...
- [x] **Track 4 — Explainer doc.** depends: Tracks 1–3 (skeleton early, figures harvested as they land). (see §5 + §1 table)
  ...
  - Done when: the doc covers all four tracks with figures/results drawn from their PRs. → **done:** `docs/ruvector-by-example.md` (linked from the README) walks the §1 feature table top to bottom — *the one idea* (no neural net; the memory is the policy), then per-feature recall → metadata → eviction → world-model → filtered recall (Track 1) → MMR (Track 1) → uncertainty/conformal (Track 3) → graph-RAG teaser — each with everyday meaning, Doom payoff, the exact call site, and a harvested proof: the `deadly_corridor` GIFs (recall), the real-Seed eviction + coverage tables (Track 2 dim/cap/eviction), the `test_aim_learning`/`test_mmr` assertions (Track 1), and the `take_cover` survival table (Track 3). Includes the honest-negatives box (inert `DbOptions.quantization` #563, dead per-query `ef_search`, rejected `agenticdb` #562 — the §0.5/§1.5 meta-finding) and a per-scenario showcase. All 59 cited tests pass; every call-site reference was adversarially fact-checked against source.
