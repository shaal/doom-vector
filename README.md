...
- **Tier 3 — runs on real hardware (Pi Zero 2 W "Cognitum Seed", 32-bit Raspbian):** the RuVector brain was cross-built for armv7 and benchmarked on the actual A53 — **~40 MiB at 20k experiences, ~270–510 recall/s (≈ decisions/s, ~30–55× the real-time bar)**. Three fixes were needed (all in the binding): cross-build for `armv7-unknown-linux-gnueabihf`; **bound `max_elements`** (default 10M pre-allocates ~661 MB → OOM); **disable the `simd` feature** (`simsimd` segfaults on armv7; scalar fallback is free at low dim). The brain is comfortably real-time on a $15 32-bit Pi. ViZDoom itself has no 32-bit wheel, so the *environment* needs 64-bit Pi OS (or a source build); the brain is proven on-device regardless. See [`deploy/pi_setup.md`](deploy/pi_setup.md).
